...
class GaussianModel:
    ...
    def set_offset(self,train_cameras):
        num_camera=len(train_cameras)
        height,width = train_cameras[0].image_height,train_cameras[0].image_width
        self._offset = [nn.Parameter(torch.zeros((3,height,width)).float().cuda().requires_grad_(True)) for _ in range(num_camera)]

    def load_offset(self,path):
        offsets = torch.from_numpy(np.load(path)).cuda()
        self._offset = [nn.Parameter(offset.requires_grad_(True)) for offset in offsets]
    ...
    def get_offset(self,cam_id):
        return self._offset[cam_id]
    ...
    def save_offset(self, offset_path):
        os.makedirs(offset_path,exist_ok=True)
        offset = torch.stack(self._offset).detach().cpu().numpy()
        np.save(os.path.join(offset_path,'offset.npy'),offset)
    ...
    def training_one_frame_s2_setup(self, training_args):
        ...
        self.offset_scheduler_new_args = get_expon_lr_func(lr_init=training_args.offset_lr_final,
                                                    lr_final=training_args.offset_lr_final,
                                                    lr_delay_mult=training_args.position_lr_delay_mult,
                                                    max_steps=100)

    def update_learning_rate_new(self, iteration):
        ''' Learning rate scheduling per step '''
        for param_group in self.optimizer.param_groups:
            if param_group["name"] == "offset":
                lr = self.offset_scheduler_ntc_args(iteration)
                param_group['lr'] = lr
    ...
    def training_one_frame_setup(self,training_args):
        ...
        l = [
            {'params': self.ntc.parameters(), 'lr': ntc_lr, "name": "ntc"},
            {'params': self._offset, 'lr': training_args.offset_lr_init, "name": "offset"}
        ]

        self.ntc_optimizer = torch.optim.Adam(l, lr=0.0)
       
        self.xyz_gradient_accum = torch.zeros((self.get_xyz.shape[0], 1), device="cuda")
        self.color_gradient_accum = torch.zeros((self.get_xyz.shape[0], 1), device="cuda")
        self.denom = torch.zeros((self.get_xyz.shape[0], 1), device="cuda")

        self.offset_scheduler_ntc_args = get_expon_lr_func(lr_init=training_args.offset_lr_init,
                                                    lr_final=training_args.offset_lr_final,
                                                    lr_delay_mult=training_args.position_lr_delay_mult,
                                                    max_steps=150)

    def update_learning_rate_ntc(self, iteration):
        ''' Learning rate scheduling per step '''
